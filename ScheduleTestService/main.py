...
def upload_test_result():
    if request.method == 'POST' and request.is_json:
        form = UpdateTestResultForm(request.get_json().get('test_datetime'), request.get_json().get('tester_id'), request.get_json().get('test_result'))
        if form.validate():
            response, http_code = update_test_result(tester_id=form.tester_id, test_datetime=form.test_datetime, test_result=form.result)
            if http_code == 200:
